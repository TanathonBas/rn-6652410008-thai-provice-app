...
import type { Recommen_tourist } from "../../types";
...
export default function TouristDetail() {
  ...
  const [item, setItem] = useState<Recommen_tourist | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setItem((data ?? null) as Recommen_tourist | null);
      } finally {
  ...
  return (
    <AnimatedScreen title={item?.name ?? "รายละเอียด"} backTarget="/tourist">
      {loading ? (
